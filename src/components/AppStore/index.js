...
class AppStore extends Component {
  ...
  render() {
    ...
    const filteredApps = this.appsList.filter(
      app =>
    ...
    return (
      <div className="app-store-container">
        ...
        <ul className="tabs-list">
          {this.tabsList.map(tabItem => (
            <li key={tabItem.tabId}>
              <button
                onClick={() => this.onClickTabItem(tabItem.tabId)}
                className={activeTabId === tabItem.tabId ? 'active-tab' : ''}
              >
                {tabItem.displayText}
              </button>
            </li>
